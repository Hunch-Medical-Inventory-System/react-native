...
export const retrieveLogs = createAsyncThunk(
  ...
  async (options: DataFetchOptions) => {
    const data = await supabaseController.readPersonalDataFromTable("logs", options, ["id", "created_at", "inventory(id, supplies(name))", "crew(first_name, last_name)", "quantity"]);
    return data;
  }
);
